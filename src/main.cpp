...
static void LedRed(){ LedSet(true,false,false); }
static void LedBlue(){ LedSet(false,false,true); }
static void LedGreen(){ LedSet(false,true,false); }
...
void loop() {
  ...
  // LED policy:
  // - Red if failure
  // - Blue while MPU changes detected
  // - Else Green (idle)
